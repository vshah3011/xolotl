...
template <typename TSpeciesEnum>
void
PSIClusterGenerator<TSpeciesEnum>::readDiffusivities(
	const std::string filename)
{
	...
	_diffusivities = Kokkos::View<double**>(
		"diffusivities", _maxV + 1, psi::getMaxHePerV(_maxV, _hevRatio) + 1);
	auto diffusivities = Kokkos::create_mirror_view(_diffusivities);
	// Initialize to 0.0
	for (IndexType i = 0; i <= _maxV; i++)
		for (IndexType j = 0; j <= psi::getMaxHePerV(_maxV, _hevRatio); j++) {
	...
	while (tokens.size() > 0) {
		if (tokens[0] <= _maxV && tokens[1] <= psi::getMaxHePerV(_maxV, _hevRatio)) {
