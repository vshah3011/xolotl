...
private:
	...
	{
		Kokkos::atomic_sub(&values(_connEntries[0]), this->_rate(gridIndex));
	}
	...
	{
		_connEntries[0] = connectivity(_reactant, _reactant);
	}
...
protected:
	...
	util::Array<IndexType, 1> _connEntries;
};

/**
 * @brief Class implementing sink reaction where
 * reactant -> product with a given rate
 *
 * @tparam TNetwork The network type
 * @tparam TDerived The derived class type.
 */
template <typename TNetwork, typename TDerived>
class DisloSinkReaction : public Reaction<TNetwork, TDerived>
{
	friend class Reaction<TNetwork, TDerived>;
...
public:
	...
	DisloSinkReaction() = default;

	KOKKOS_INLINE_FUNCTION
	DisloSinkReaction(ReactionDataRef reactionData,
	...
	DisloSinkReaction(ReactionDataRef reactionData,
		...
		DisloSinkReaction(reactionData, clusterData, reactionId,
...
protected:
	...
	{
		Kokkos::atomic_sub(&values(_connEntries[0]), this->_rate(gridIndex));
		Kokkos::atomic_add(&values(_connEntries[1]), this->_rate(gridIndex));
	}
		...
		Kokkos::View<double*> values, IndexType gridIndex)
	{
		...
		if (_product == _reactant)
			Kokkos::atomic_add(
				&values(_connEntries[1]), this->_rate(gridIndex));
	}
	...
	{
		_connEntries[0] = connectivity(_reactant, _reactant);
		_connEntries[1] = connectivity(_product, _reactant);
	}
...
protected:
	...
	util::Array<IndexType, 2> _connEntries;
};
 ...
 */
template <typename TNetwork, typename TDerived>
class GBSinkReaction : public Reaction<TNetwork, TDerived>
{
	friend class Reaction<TNetwork, TDerived>;
...
public:
	...
	GBSinkReaction() = default;

	KOKKOS_INLINE_FUNCTION
	GBSinkReaction(ReactionDataRef reactionData, const ClusterData& clusterData,
	...
	GBSinkReaction(ReactionDataRef reactionData, const ClusterData& clusterData,
		IndexType reactionId, const detail::ClusterSet& clusterSet) :
		GBSinkReaction(reactionData, clusterData, reactionId,
			clusterSet.cluster0, clusterSet.cluster1)
	{
	}
