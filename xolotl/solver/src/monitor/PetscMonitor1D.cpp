...
PetscMonitor1D::setup(int loop)
{
	...
	PetscBool flagNeg, flagCollapse, flag2DPlot, flag1DPlot, flagSeries,
		...
		flagAlloy, flagDisl, flagTemp, flagLargest, flagZr;
	...
	PetscCallVoid(PetscOptionsHasName(NULL, NULL, "-disl_profile", &flagDisl));
	...
	if (flagDisl) {
		...
		PetscCallVoid(
			TSMonitorSet(_ts, monitor::profileDislocation, this, nullptr));
	}
	...
	TS ts, PetscInt timestep, PetscReal time, Vec solution)
{
	...
	for (auto i = xs; i < xs + xm + 2; i++) {
		...
		localDislocation.push_back(
			gridPointSolution[dof - 3]); // Dislocation density
	}
